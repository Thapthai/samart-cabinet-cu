...
import WeighingDispenseFiltersToolbar, {
  WeighingDispenseReportDownloadButtons,
} from './WeighingDispenseFiltersCard';
...
function weighingOperatorLabel(row: WeighingDispenseDetailRow): string {
...
export default function WeighingDispenseTableCard({
  ...
  const toolbar = (
    <WeighingDispenseFiltersToolbar
      ...
      reportActions={
        <WeighingDispenseReportDownloadButtons
          ...
          showCombined={Boolean(onDownloadDispensedAllExcel)}
          onDownloadExcel={onDownloadExcel}
          onDownloadPdf={onDownloadPdf}
          onDownloadDispensedAllExcel={onDownloadDispensedAllExcel}
        />
      }
    />
  );
            ...
            <StockMobileCardList>
              {tableMode === 'WEIGHING' &&
                (items as WeighingDispenseDetailRow[]).map((row, index) => {
                  ...
                  const cabinet = weighingDetailCabinetLabel(row, cabinetDisplayFallback);
              ...
              {tableMode === 'RFID' &&
                (items as RfidDispensedListRow[]).map((row, index) => {
                  ...
                  const cabinet = rfidCabinetLabel(row);
                    ...
                    (items as WeighingDispenseDetailRow[]).map((row, index) => (
                      <TableRow key={`w-${row.id}-${index}`} className="border-b border-border/50 hover:bg-muted/40">
                        ...
                        <TableCell
                          ...
                          title={weighingDetailCabinetLabel(row, cabinetDisplayFallback)}
                        >
